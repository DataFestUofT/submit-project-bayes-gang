...
### Total unemployment rate 2019 for major industries
hist(as.numeric(data_ind[3,major_ind]))
### Total unemployment rate 2020 for major industries
hist(as.numeric(data_ind[4,major_ind]))
...
sidebar <-  dashboardSidebar(
  sidebarMenu(
    ...
    menuItem("Conclusions", tabName = 'limits', icon = icon("exclamation"))
  )
)
...
body <- dashboardBody(
  tabItems(
    tabItem(tabName = "background",
            ...
            Businesses, especially small ones, were suffering from the lack of customers and thus having trouble paying their employees."),
            p("As a result, there were mass layoffs and employment plummetted."),
            p(),
            fluidRow(
            ...
            tags$li("Those requiring human contact would suffer more", 
              span(em("(i.e. service related or restaurant)")), 
              "or, those jobs where employees were less than 6ft apart",
              span(em("(i.e. manufacturing)."))),
            tags$li("However, work done in an more formal office setting should be easier to resume online and at home."),
            p(),
            p("Also, we decided that other factors could also contribute to the job loss."),
            tags$li("Sex"),
            tags$li("Health (both COVID related and not)"),
            tags$li("Education level"),
            tags$li("Income"),
            p(),
            p("Here is a visualization of the job loss by industry and per state"),
            p(),
            fluidRow(
              box(checkboxGroupInput("morf","Total or by Sex",choices = c("Total" = "total", "Male" = "male", "Female" = "female"), selected = "total"),
            )),
            fluidRow(
              plotOutput("byindustry",height = 900,width = 1000)
            ),
            tags$style(type='text/css', "#button { vertical-align: middle; height: 50px; width: 100%; font-size: 30px;}"),
            p(),
            p("We also thought it would be very telling to look at the stay at home orders per state as they could be indicative of job loss. Please hover over the state to see which date a stay at home order was called."),
    ...
    tabItem(tabName = "researchq",
            ...
            h3("Our Hypotheses:"),
            p("1. We hypothesize that there will be a positive correlation between number of cases within an area and low income job loss rate (LI JLR)."),
            p("2. We hypothesize that more educated population corresponds to a lower LI JLR."),
            p("3. We hypothesize that there is a negative correlation between income and the LI JLR."),
            p("4. We hypothesize that as the overall health of the population decreases the JLR will increase."),
            p("5. We hypothesize that ‘Blue’ states have higher JLR than ‘Red’ states and this might be because ‘blue’ states were quicker to lockdown.")
              ),
    ...
    tabItem(tabName = "conclusions",
            ...
            fluidRow(
              box(plotOutput("correlations"),width = 12,height = NULL)),
            p("Something about hypotheses 1-4"),
            ...
            p("Here is the average low income job loss by state based on our model. Hovering over each state you will see the average Job loss rate and the date that the state issued a lockdown."),
            ...
            fluidRow(
              box(plotOutput("numdays"),width = 12,height = NULL)
            ),
            ...
            p(),
            fluidRow(
              box(plotOutput("dem"),width = 12,height = NULL)),
            p(),
            fluidRow(
              box(plotOutput("rep"),width = 12,height = NULL)),
            p("From these graphs, we see that as the majority of the state who views align with the Democratic party, LI JLR increased. Meanwhile, we see the opposite trend occuring for those states where the majority aligns with the Republican party.")
            ),
    tabItem(tabName = 'limits',
            h3("Conclusions"),
            p("Percentage of COVID-19 cases per state was not a good predictor of low income JLR using current data"),
            p("State ‘politics’, including number of days non-essential businesses were closed down and state political leaning were more indicative of low income JLR."),
            tags$li("States with earlier closed-down dates suffered from greater LI JLR."),
            tags$li("States that are more liberal leaning suffered from greater LI JLR."),
            h3("Limitations"),
            p("Our model is poorly indicative of which factors affected JLR because the increase in JLR between February 2020 and April 2020 was so drastic in such a short period of time and effectively shocked the labor market.  If we were to re-do this project in the future, the model might be able to predict which factors can affect the unemployment rate. Also, we would be able to see the re-employment rate for when things go back to “normal”."),
            p("We looked only at low income JLR under the assumption that these jobs would exhibit more drastic change in JLR. However, high income JLR may also be worth looking into as they may have different factors contributing to unequal JLR."),
            p('We only considered the JLR between February 2020 and April 2020'),
            h3("Future Directions"),
            p("Can include number of days in a 'locked down' state and political leaning for each state as predictors in the model to investigate their effect on low income JLR."),
            p("Model on high income JLR and compare if any significant predicting factors act similarly or differently from low income JLR."),
            p("In the future when the virus is better controlled and if the economy recovers, we can look at how different factors affect re-employment rate and compare with our findings here to look for factors with stronger causal relationship to JLR.")
    ...
    tabItem(tabName = "model",
            ...
            p("The fixed effects estimates in this model represent 
             ...
             p(" The random intercepts are displayed below on the map of each of the 50 states. 
              This intercept can be interpreted as the average job loss rate by 
              State, since all the other variables are centered in the model. State random intercept (between-group variation) explained approximately 86.2% of total variation in the model, justifying its inclusion in the model. Features were not inter-correlated and small standard error suggest lack of multicollinearity and strong validity of parameter estimates."),
            p("The following covariates were added to the model: "),
            tags$li("Number of COVID-19 cases per population"),
            ...
            p(),
            p("The plot below shows the point estimates for the coefficients of the model as well as error bars representing a deviation of 2 standard errors."),
            p(),
            fluidRow(
              plotlyOutput("coeffs")
            ),
            p(),
            p("Each coefficient is scaled and the outcome is on a logit scale, so to report each coefficient, the logit inverse needs to be applied. 
              We can see that fair or poor health and high school graduation rate are positively associated with JLR, meanwhile poverty is negatively associated. 
...
server <- function(input, output) {
    output$unemployment <- renderPlot(
      ggplot(data_work_class, aes(x=mon_year, y=total, group=1)) +
        geom_line() +
        ...
        scale_x_discrete(limits=data_work_class$mon_year) +
        theme(axis.text.x = element_text(angle = 45, hjust = 1),
              axis.text.y = element_text(angle = 0, hjust = 1),
              plot.title = element_text(hjust = 0.5))
    )
    output$byindustry <- renderPlot(
      unemp_total_long %>% filter(.data[["section"]] %in% input$morf) %>% 
          ggplot(aes(x=industry, y=rate, group = section, fill=section)) +
          geom_bar(stat = 'identity', position="dodge") + 
          xlab(NULL) +
          ggtitle(c('Unemployment rate increase in major industries \nApr 2019 vs. Apr 2020')) +
      ...
      output$numdays <- renderPlot(
        nofdays%>% ggplot(aes(Days.to.Issue.Stay.at.home.from.National.Announcment,intercept))+ geom_jitter()+geom_text(aes(label=STATE), size=3, 
                                                                                                                       check_overlap = T,nudge_y=0.005)+geom_smooth(method=lm)  +labs(title ="Low income job loss rate by days to issue state order", x = "Days to issue state order", y = "Estimated Intercept of LI JLR", caption = "Source: Business Insider, Al Jazeera " )
      )
